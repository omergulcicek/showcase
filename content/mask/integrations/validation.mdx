...
ViraMask is designed to work seamlessly with popular validation libraries like **Zod** and **Yup**, as well as standard **React Hook Form** validation.

ViraMask exports its internal validator functions so you can use them directly in your schemas:
...
Use Zod's `refine` method to apply ViraMask's validators.
...
Use Yup's `test` method to apply ViraMask's validators.
...
> **Note:** When using `useViraMask` without a resolver, ViraMask automatically injects its default validators into the `register` function. However, you can override or extend this behavior manually if needed.
...
export default function StandardForm() {
  ...
  const { tckn } = useViraMask({
    ...
    schema: {
      tckn: {
        preset: 'tckn',
        // ViraMask handles this automatically, but here is how you'd do it manually:
