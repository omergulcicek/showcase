      ...
      <div className="container mx-auto">
        <div className="flex flex-col items-center justify-center gap-4 mt-40">
          ...
          <p className="max-w-4xl text-center text-muted-foreground">
            Seamlessly integrate smart masking and robust validation into your{" "}
            <span className="font-medium text-primary">react-hook-form</span>{" "}
            inputs. Get clean, unmasked values with full{" "}
            <span className="font-medium text-primary">TypeScript</span>{" "}
            support, built to work perfectly with{" "}
          ...
          >
            <div className="flex flex-col gap-6 mb-20">
              <div className="grid gap-2 relative">
                ...
                <figure className="absolute -left-18 top-10 select-none">
                  <ArrowIcon className="size-14" />
                  <figcaption className="-ml-5">You try</figcaption>
                </figure>

                <div className="border border-dashed text-xs p-2 bg-muted/50">
                  <div className="flex items-center justify-between gap-2">
                    <code>{`{cardNumber.value}`}</code>
                    <span>{cardNumber.value}</span>
                  </div>
                  <div className="flex items-center justify-between gap-2">
                    <code>{`{cardNumber.maskedValue}`}</code>
                    <span>{cardNumber.maskedValue}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-start gap-2">
                <div className="grid gap-2 w-2/3">
                  ...
                  <Input {...expiryDate} id="expiryDate" placeholder="MM/YY" />

                  <div className="border border-dashed text-xs p-2 bg-muted/50">
                    <div className="flex items-center justify-between gap-2">
                      <code>{`{expiryDate.value}`}</code>
                      <span>{expiryDate.value}</span>
                    </div>
                    <div className="flex items-center justify-between gap-2">
                      <code>{`{expiryDate.maskedValue}`}</code>
                      <span>{expiryDate.maskedValue}</span>
                    </div>
                  </div>
                </div>
            ...
            <div className="grid gap-2">
              ...
              <Input {...phone} id="phone" placeholder="(5xx) xxx xx xx" />
              ...
              <div className="border border-dashed text-xs p-2 bg-muted/50">
                ...
                <div className="flex items-center justify-between gap-2">
                  <code>{`{phone.maskedValue}`}</code>

                  <span>{phone.maskedValue}</span>
                </div>
              </div>
            </div>

            <div className="grid gap-2">
              <Label htmlFor="alpha">Alpha (only letters)</Label>
              <Input {...alpha} id="alpha" placeholder="John Doe" />
            </div>

            <div className="grid gap-2">
              <Label htmlFor="tckn">TCKN</Label>
              <Input {...tckn} id="tckn" placeholder="12345678950" />
            </div>

            <div className="grid gap-2">
              <Label htmlFor="email">Email</Label>
              <Input {...email} id="email" placeholder="email@example.com" />
            </div>

            <div className="grid gap-2">
              <Label htmlFor="url">URL</Label>
              <Input {...url} id="url" placeholder="https://example.com/" />
            </div>

            <div className="grid gap-2">
              <Label htmlFor="password">Password</Label>
              <Input {...password} id="password" placeholder="••••••••" />
            </div>

            <div className="grid gap-2">
              <Label htmlFor="details">Details (free text)</Label>
              <Input
                {...details}
                id="details"
